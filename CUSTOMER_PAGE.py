...
#==============================Show data table===================          
        ...
        try:
            if self.var_mobile.get()=="" or self.var_mother.get()=="":
                ...
                my_cursor.execute("insert into customer values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                     self.var_ref.get(),self.var_cust_name.get(),self.var_mother.get(),self.var_gender.get(),self.var_post.get(),self.var_mobile.get(),
                     self.var_email.get(),self.var_nationality.get(),self.var_idproof.get(),self.var_idnumber.get(),self.var_address.get()
                     ))
                conn.commit()
                self.fetch_data()
                self.reset()
    ...
    def update(self):
        if self.var_mobile.get() == "":
            ...
            conn = mysql.connector.connect(host="localhost", user="root", password="123456", database="hotelmanagement")
            my_cursor = conn.cursor()
            query = "UPDATE customer SET name=%s, mother=%s, gender=%s, postcode=%s, Mobile=%s, Email=%s, Nationality=%s, Idproof=%s, Idnumber=%s, Address=%s WHERE ref=%s"
            values = (
